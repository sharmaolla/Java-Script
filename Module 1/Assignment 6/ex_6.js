/*Write a program that prints the text "Should I calculate the square root?"
 ...
 and calculates and prints its square root to the HTML document. If the user selects Cancel,
...
You cannot calculate the square root of a negative number. If the number entered by the user
is negative, the program prints "The square root of a negative number is not defined" to the HTML document. */
...
if (question) {
    ...
    if (number >= 0) {
       document.querySelector('#target').innerHTML = `The quare root of ${number} is ${Math.sqrt(number)}`;
    }
    else {
